...
void OneWireAnalyzer::WorkerThread()
{
    ...
    for( ;; )
    {
        if( mBlockPulseAdvance == false )
        {
            ...
            // At this point, we should be sitting on a negative edge, and have the pulse length of the low pulse in front, and the high
            // pulse behind.
        }
        mBlockPulseAdvance = false;
        ...
        case RomCommandDetectedState:
        {
            ...
            case ReadRom:
            {
                ...
                if( mRomBitsRecieved == 56 )
                {
                    ...
                    // TODO:  RecordBubble( one_wire_bubbles, mByteStartSample, mPreviousRisingEdgeSample, Rom, ((mRomDetected >> 8) &
                    // 0xFFFFFFFFFFFFull) ); RecordFrame( mByteStartSample, mPreviousRisingEdgeSample, Rom, ((mRomDetected >> 8) &
                    // 0xFFFFFFFFFFFFull) );
                    mByteStartSample = mFallingEdgeSample;
                }
...
U32 OneWireAnalyzer::GenerateSimulationData( U64 minimum_sample_index, U32 device_sample_rate,
